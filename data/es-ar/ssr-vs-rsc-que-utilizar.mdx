...
Anteriormente, en la manera tradicional de renderizar una aplicación de React, el servidor devolvía un paquete de Javascript que se encargaba de renderizar la aplicación al ser ejecutada en el navegador. Los problemas conocidos de esta estrategia siempre fueron los mismos: SEO, rendimiento, tiempo de carga, e incluso más.
...
El momento previo al renderizado es el momento perfecto para realizar petición de datos, como hacer una petición a la base de datos y crear el HTML en base a ello aprovechando que ya estamos en el servidor.
...
Existen restricciones a la hora de utilizar RSC, como por ejemplo, no pueden pasarse algunos tipos de props (explicado más adelante), y el hecho de no poder importar componentes de servidor dentro de componentes de cliente, pero si viceversa. Esto se debe a que estos componentes pueden tener datos sensibles o código que directamente no corre en el navegador.
